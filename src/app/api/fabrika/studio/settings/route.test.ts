...
describe('/api/fabrika/studio/settings', () => {
  afterEach(() => {
    vi.unstubAllEnvs();
    vi.clearAllMocks();
  });

  it('her oturum sahibine yalnız maskesiz platform hazır bilgisini verir', async () => {
    vi.stubEnv('STABILITY_API_KEY', 'sk-platform-stability');
  ...
  it('müşterinin sağlayıcı anahtarı yazmasını reddeder', async () => {
    ...
    const response = await PUT(
      new Request('https://example.test/api/fabrika/studio/settings', {
        method: 'PUT',
        body: JSON.stringify({ provider: 'GEMINI', apiKey: 'customer-secret' }),
      })
    );
