...
<body>
  ...
  <footer id="iletisim"><strong>${companyName}</strong><span>Profesyonel gayrimenkul danışmanlığı</span></footer>
    ...
    const generatedSite = await prisma.$transaction(async (tx) => {
      const site = await tx.generatedWebsite.create({
        data: {
      ...
      await tx.websitePromptVersion.create({
        data: {
          companyAccountId: account.id,
          generatedWebsiteId: site.id,
          version: 1,
          promptTemplate: connectorPrompt,
          source: "GENERATED_WEBSITE_CREATED",
          createdByType: "OWNER",
