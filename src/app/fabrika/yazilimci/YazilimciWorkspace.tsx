...
type YazilimciPageProps = {
  initialData?: HubData;
};

export function YazilimciWorkspace({ initialData }: YazilimciPageProps = {}) {
  ...
  const [mode, setMode] = useState<WebsiteMode | null>(
    initialData && initialData.website.mode !== "UNDECIDED"
      ? initialData.website.mode
      : null,
  );
  const [baseDomain, setBaseDomain] = useState(
    initialData ? baseDomainFromHostname(initialData.website.customHostname) : "",
  );
  ...
  const [socialDraft, setSocialDraft] = useState<SocialAccountNote>(() =>
    initialData?.socialAccounts.find((account) => account.platform === "instagram") ??
  ...
  return (
    <div
      ...
      data-visual-theme="studio"
      data-yazilimci-workspace="true"
    >
            ...
            <div className={styles.studioLayout}>
              <aside className={styles.creationRail}>
                ...
                <button className={styles.createWebsiteCard} onClick={() => openWebsiteBuilder("NEW")} type="button">
                  <span className={styles.actionKicker}>ÖNERİLEN</span>
                  ...
                  <span className={styles.actionArrow} aria-hidden="true"><ArrowRight /></span>
                </button>

                <button className={styles.connectWebsiteCard} onClick={() => openWebsiteBuilder("EXISTING")} type="button">
                  <span className={styles.actionKicker}>ALAN ADIM VAR</span>
                  ...
                  <span className={styles.connectionMiniFlow} aria-hidden="true">
                    <i>sitem.com</i><ArrowRight /><i>CNAME</i><ArrowRight /><i>CEO AI</i>
                  </span>
                  <span className={styles.actionArrow} aria-hidden="true"><ArrowRight /></span>
                </button>

                <button className={styles.socialSetupCard} onClick={openSocialWorkspace} type="button">
                  <span className={styles.actionKicker}>HESAPLARI HAZIRLA</span>
                  <span className={styles.actionIcon}><UserRound /></span>
                  <span className={styles.actionCopy}>
                    <strong>Sosyal Medya Hesaplarını Kur</strong>
                    <small>Sosyal medya hesaplarınızı bağlayın ve otomatik senkronizasyonu açın.</small>
                  </span>
                  ...
                  <span className={styles.socialProgress}>
                    <b>{Math.min(savedSocialCount, 4)} / 4 hesap bağlı</b>
                    <i><em style={{ width: `${Math.min(savedSocialCount, 4) * 25}%` }} /></i>
                  </span>
                  <span className={styles.actionArrow} aria-hidden="true"><ArrowRight /></span>
                </button>
              </aside>
              ...
              <main className={styles.websiteGallery}>
                <header className={styles.galleryHeader}>
                  <div>
                    ...
                    <p>100 profesyonel, çok sayfalı tasarımdan birini seçin.</p>
                  </div>
                  <div className={styles.galleryHeaderActions}>
                    <span className={styles.themeCount}>100 TASARIM</span>
                ...
                <div className={styles.galleryControls}>
                  ...
                  <button
                    ...
                    onClick={() => setWebsiteCardFilter("templates")}
                    type="button"
                  >
                    <SlidersHorizontal /> Filtrele
                  </button>
                  <div className={styles.viewToggle} role="group" aria-label="Görünüm">
                    <button aria-label="Kart görünümü" aria-pressed={websiteCardView === "grid"} onClick={() => setWebsiteCardView("grid")} type="button"><Grid2X2 /></button>
                    <button aria-label="Liste görünümü" aria-pressed={websiteCardView === "list"} onClick={() => setWebsiteCardView("list")} type="button"><List /></button>
                  </div>
                </div>
                ...
                {studioThemes.length ? (
                  <div className={styles.websiteCards} data-view={websiteCardView}>
                    {studioThemes.map((theme) => {
                      ...
                      const previewSlot = DEVELOPER_THEMES.findIndex((item) => item.id === theme.id) % 6;
                      return (
                        <article
                          className={styles.websiteCard}
                          data-current={isCurrent}
                          data-density={theme.design.density}
                          data-hero={theme.design.hero}
                          data-layout={theme.layout}
                          data-navigation={theme.design.navigation}
                          data-portfolio={theme.design.portfolio}
                          data-preview-slot={previewSlot}
                          data-shape={theme.design.shape}
                          data-theme={theme.id}
                          data-typography={theme.design.typography}
                          key={theme.id}
                          style={{
                        ...
                        >
                          ...
                          <div className={styles.websiteCardBody}>
                            ...
                            <div className={styles.cardDetails}>
                              ...
                              <span className={styles.paletteSwatches} aria-label={`${theme.name} renk paleti`}>
                                <i style={{ background: theme.colors.background }} />
                                <i style={{ background: theme.colors.accent }} />
                                <i style={{ background: theme.colors.muted }} />
                              </span>
                            </div>
                          </div>
                          ...
                          <footer className={styles.websiteCardFooter}>
                            ...
                            {isCurrent && website.status === "PUBLISHED" ? (
                              ...
                              <button aria-label={`${theme.name} ayrıntıları`} onClick={() => openWebsiteBuilder(isCurrent ? mode ?? "NEW" : "NEW", theme.id)} type="button"><Eye /></button>
                            )}
                          </footer>
                        </article>
                      );
                    ...
                    <div className={styles.themePickerHead}>
                      <div>
                        ...
                        <span>
                          ...
                          <small>100 tasarımın hero kurgusu, menüsü, tipografisi ve portföy sunumu birbirinden farklıdır; her biri 7 sayfalıdır.</small>
                        </span>
                      </div>
                      <label>
                        <Search />
