...
const stageLabels: Record<string, string> = { DISCOVERED: 'Keşfedildi', QUALIFIED: 'Nitelikli', CONTACTED: 'İletişim kuruldu', ENGAGED: 'Yanıt verdi', MEETING: 'Görüşme', REVIEW: 'İnceleme', AGREEMENT: 'Sözleşme', ACTIVE: 'Aktif', DISQUALIFIED: 'Uygun değil', NOT_INTERESTED: 'İlgilenmiyor', DO_NOT_CONTACT: 'İletişim kurma', ARCHIVED: 'Arşiv' };
...
export default function PartnerNetworkClient({ initialPartners, owner }: { initialPartners: Partner[]; owner: boolean }) {
  ...
  const filtered = useMemo(() => partners.filter((partner) => {
    ...
    return (!country || partner.countryCode === country) && (!query || `${partner.displayName} ${partner.city || ''} ${partner.countryName}`.toLocaleLowerCase('tr-TR').includes(query));
  }), [partners, search, country]);
  const countries = useMemo(() => [...new Map(partners.map((partner) => [partner.countryCode, partner.countryName])).entries()], [partners]);
  const metrics = {
    candidates: partners.filter((p) => !['ACTIVE', 'ARCHIVED', 'DO_NOT_CONTACT'].includes(p.stage)).length,
    approval: partners.reduce((sum, p) => sum + (p.contacts.some((c) => c.verificationStatus.includes('VERIFIED')) ? 1 : 0), 0),
    contacted: partners.filter((p) => ['CONTACTED', 'ENGAGED', 'MEETING', 'REVIEW', 'AGREEMENT', 'ACTIVE'].includes(p.stage)).length,
  ...
  const listForTab = tab === 'active' ? filtered.filter((p) => p.stage === 'ACTIVE') : filtered;
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      {['candidates','approval','pipeline','active'].includes(tab) && <section className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-500"/><input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Kurum, şehir veya ülke ara" className="w-full rounded-lg border border-slate-700 bg-slate-950 py-2.5 pl-10 pr-3 text-sm text-white outline-none focus:border-emerald-500"/></label><select value={country} onChange={(e)=>setCountry(e.target.value)} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white"><option value="">Tüm ülkeler</option>{countries.map(([code,name])=><option key={code} value={code}>{name}</option>)}</select></div>{listForTab.length===0?<div className="rounded-xl border border-dashed border-slate-700 py-16 text-center"><CircleOff className="mx-auto h-8 w-8 text-slate-600"/><p className="mt-3 font-semibold text-slate-300">Bu görünümde kayıt yok</p><p className="mt-1 text-sm text-slate-500">Kaynak ekleyin veya filtreleri değiştirin.</p></div>:<div className="grid gap-3">{listForTab.map((partner)=><button key={partner.id} onClick={()=>openPartner(partner.id)} className="grid gap-4 rounded-xl border border-slate-800 bg-slate-900 p-4 text-left transition hover:border-emerald-500/50 sm:grid-cols-[1fr_auto] sm:items-center"><div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-emerald-500/10 text-emerald-400"><Building2 className="h-5 w-5"/></div><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-white">{partner.displayName}</h3><span className="rounded-full bg-slate-800 px-2 py-1 text-[11px] text-slate-300">{stageLabels[partner.stage]||partner.stage}</span></div><p className="mt-1 text-sm text-slate-400">{partner.city ? `${partner.city}, `:''}{partner.countryName} · {partner.specialties.slice(0,2).join(' · ')||'Uzmanlık belirtilmedi'}</p><p className="mt-2 text-xs text-slate-500">Kaynak: {partner.sources[0]?.title||'Başvuru'} · {relativeDate(partner.lastVerifiedAt)}</p></div></div><div className="flex items-center gap-5"><div className="text-right"><p className="text-xs text-slate-500">Uygunluk / Güven</p><p className="font-bold text-white">{Math.round(partner.fitScore)} / %{Math.round(partner.confidenceScore)}</p></div>{busy===`partner:${partner.id}`?<Loader2 className="h-5 w-5 animate-spin text-emerald-400"/>:<ChevronRight className="h-5 w-5 text-slate-500"/>}</div></button>)}</div>}</section>}

      {selected && <div className="fixed inset-0 z-[80] flex justify-end bg-slate-950/70" onMouseDown={(e)=>{if(e.currentTarget===e.target)setSelected(null)}}><aside className="h-full w-full max-w-3xl overflow-y-auto border-l border-slate-800 bg-[#0b1322] p-5 shadow-2xl sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-400">{selected.countryName}</p><h2 className="mt-1 text-2xl font-black text-white">{selected.displayName}</h2><p className="mt-1 text-sm text-slate-400">{selected.city||'Şehir belirtilmedi'} · Uygunluk {Math.round(selected.fitScore)} · Güven %{Math.round(selected.confidenceScore)}</p></div><button onClick={()=>setSelected(null)} className="rounded-lg border border-slate-700 p-2 text-slate-400 hover:text-white"><X className="h-5 w-5"/></button></div><div className="mt-6 grid gap-3 sm:grid-cols-2"><select value={selected.stage} onChange={(e)=>updateStage(e.target.value)} disabled={busy==='stage'} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white">{stages.map((stage)=><option key={stage} value={stage}>{stageLabels[stage]}</option>)}</select><button disabled={busy==='draft'} onClick={createDraft} className="flex items-center justify-center gap-2 rounded-lg bg-emerald-500 px-4 py-2.5 text-sm font-bold text-slate-950 disabled:opacity-50">{busy==='draft'?<Loader2 className="h-4 w-4 animate-spin"/>:<Mail className="h-4 w-4"/>} Kaynaklı e-posta hazırla</button></div>{selected.websiteUrl&&<a href={selected.websiteUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-cyan-300">Kurumsal siteyi aç <ExternalLink className="h-4 w-4"/></a>}
        <section className="mt-7"><h3 className="font-bold text-white">Kurumsal iletişim</h3><div className="mt-3 space-y-2">{selected.contacts.map((contact)=><div key={contact.id || contact.emailMasked} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-800 bg-slate-900 p-3"><div><p className="text-sm font-semibold text-slate-200">{contact.emailMasked || 'Adres maskelendi'}</p><p className={`mt-1 text-xs ${contact.verificationStatus.includes('VERIFIED') ? 'text-emerald-300' : contact.verificationStatus === 'REJECTED' ? 'text-rose-300' : 'text-amber-300'}`}>{contact.verificationStatus.includes('VERIFIED') ? 'Gönderime uygun kurumsal adres' : contact.verificationStatus === 'REJECTED' ? 'Reddedildi' : 'Patron doğrulaması bekliyor'}</p></div>{owner && contact.id && !contact.verificationStatus.includes('VERIFIED') && <div className="flex gap-2"><button disabled={busy===`contact:${contact.id}`} onClick={()=>verifyContact(contact.id!, 'VERIFY')} className="rounded-lg bg-emerald-500 px-3 py-2 text-xs font-bold text-slate-950 disabled:opacity-50">Doğrula</button><button disabled={busy===`contact:${contact.id}`} onClick={()=>verifyContact(contact.id!, 'REJECT')} className="rounded-lg border border-rose-500/30 px-3 py-2 text-xs font-semibold text-rose-200 disabled:opacity-50">Reddet</button></div>}</div>)}{selected.contacts.length===0&&<p className="text-sm text-slate-500">Kurumsal iletişim bulunamadı.</p>}</div></section>
        <section className="mt-7"><h3 className="font-bold text-white">Doğrulanmış kaynaklar</h3><div className="mt-3 space-y-2">{selected.sources.map((source)=><div key={source.id} className="rounded-lg border border-slate-800 bg-slate-900 p-3"><div className="flex items-center justify-between gap-3"><span className="text-sm font-semibold text-slate-200">{source.title||source.type}</span><span className={`text-xs ${source.trusted?'text-emerald-300':'text-amber-300'}`}>{source.trusted?'Doğrulanmış':'Manuel inceleme'}</span></div>{source.sourceUrl&&<a className="mt-2 block truncate text-xs text-cyan-300" href={source.sourceUrl} target="_blank" rel="noreferrer">{source.sourceUrl}</a>}</div>)}</div></section>
        ...
        <section className="mt-7"><h3 className="font-bold text-white">Geçmiş</h3><div className="mt-3 space-y-2">{selected.activities.map((activity)=><div key={activity.id} className="border-l-2 border-slate-700 pl-3"><p className="text-sm text-slate-200">{activity.summary}</p><p className="text-xs text-slate-500">{relativeDate(activity.createdAt)}</p></div>)}</div></section>
      </aside></div>}
    </div>
  );
}
