  ...
  const createPoster = async () => {
    ...
    try {
      ...
      const background = await resolvePosterBackground({
        mode: form.mode,
        localBackgroundUrl: selectedPosterSources[0].url,
        request: async () => {
      ...
      const effectiveForm = { ...form, mode: background.effectiveMode };
      ...
      if (background.fallbackUsed) {
        toast(background.warning || 'AI servisi yanıt vermedi; poster mevcut fotoğrafla hazırlandı.');
      } else {
