...
export default function WhatsAppCRM({
  ...
  async function fetchGatewayStatus() {
    try {
      const res = await fetch('/api/fabrika/whatsapp/connection', {
        cache: 'no-store',
      });
      if (res.ok) {
  ...
  const handleBulkSend = async () => {
    toast.success('WhatsApp otonom gönderimi başlatıldı...');
      ...
      {gatewayStatus && !liveConfigured && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 mb-4 flex items-center justify-between backdrop-blur-xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h5 className="text-sm font-bold text-amber-200">WhatsApp Telefon Bağlantısı Yapılmadı</h5>
              <p className="text-xs text-amber-300/80 mt-0.5">
                Canlı mesaj göndermek için WhatsApp Merkezi&apos;nden şirket telefonunu QR kodla bağlayın.
              </p>
            </div>
          </div>
          {permissions.canManageSecrets ? (
            <a href="/fabrika/whatsapp" className="text-xs font-bold text-amber-950 bg-amber-400 hover:bg-amber-300 px-4 py-2 rounded-xl transition-all shadow-md">
              Telefonu Bağla
            </a>
          ) : (
          ...
          {isBulkSending ? (
            <div className="space-y-2 bg-gray-950/50 p-3 rounded-xl border border-gray-800">
              <div className="flex justify-between text-xs text-gray-400">
                <span>WhatsApp gönderimi devam ediyor...</span>
              </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
            {waChats.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-500 space-y-3 p-6 text-center">
                  ...
                  <p className="text-xs text-gray-600">Gelen mesajlar güvenli webhook ile buraya otomatik düşer.</p>
              </div>
            ) : (
          ...
          {waSelectedChat ? (
                    ...
                    {isDraft && (
                        <div className="flex gap-2 mt-2 max-w-[75%]">
                            ...
                            <button onClick={() => handleApproveDraft(msg.id, msg.content)} className="flex-1 flex justify-center items-center gap-2 px-6 py-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white text-sm font-bold transition-all shadow-[0_0_15px_rgba(16,185,129,0.3)] hover:-translate-y-0.5">
                                <Send className="w-4 h-4" /> Onayla ve WhatsApp ile Gönder
                            </button>
                        </div>
                    )}
                  </div>
            ...
            <div className="relative flex flex-col h-full z-10 items-center justify-center">
              <div className="text-center p-10 max-w-md animate-in zoom-in-95 duration-700">
                ...
                <h3 className="text-3xl font-light text-gray-300 mb-4 tracking-tight">Jasmine WhatsApp CRM</h3>
                ...
                <div className="mt-10 inline-flex items-center gap-2 text-xs text-gray-400 bg-gray-900/50 px-4 py-2 rounded-full border border-gray-800">
                    <ShieldCheck className="w-4 h-4 text-emerald-400" /> Şirket WhatsApp Hattı Entegre
                </div>
              </div>
            </div>
          )}
