...
async function processLiveJob(job: NonNullable<JobWithAuthorization>) {
  ...
  const { sourceSessionId, robotsFile, firstNavigationAt } =
  ...
  const crawler = new PlaywrightCrawler({
    ...
    // The legacy self-hosted worker remains available only for fixture/backfill
    // compatibility. apify and @crawlee/playwright currently resolve separate
    // copies of Crawlee's nominal ProxyConfiguration class; their runtime
    // contract is the same, so bridge that package-level type mismatch here.
    proxyConfiguration:
      proxyConfiguration as unknown as NonNullable<
        ConstructorParameters<typeof PlaywrightCrawler>[0]
      > extends infer Options
        ? Options extends { proxyConfiguration?: infer Proxy }
          ? Proxy
          : never
        : never,
    ...
    preNavigationHooks: [
      async ({ request }) => {
        ...
        await waitUntil(firstNavigationAt);
      },
    ],
